...
export default function TodayAcademicSchedule({
  ...
  return (
    <section aria-label="Today's Academic Schedule" className="space-y-2 pt-1">
            ...
            return (
              ...
              >
                <div className="flex items-baseline gap-2.5 min-w-0">
                  <span className="text-neutral-400 font-medium whitespace-nowrap text-xs">
                  ...
                  {item.location && (
                    <span className="text-xs text-neutral-600 hidden sm:inline truncate">
                      ({item.location})
                    </span>
                  )}
                </div>
                ...
                <div className="text-right whitespace-nowrap">
                  {isCancelled ? (
                    <span className="rounded bg-rose-950/80 px-1.5 py-0.5 text-xs font-medium text-rose-300 border border-rose-800/50">
                      Cancelled
                    </span>
                  ) : item.status === "held" ? (
                    <span className="text-xs text-emerald-400">Held</span>
                  ) : (
                    <span className="text-xs text-neutral-400">{item.endTime.slice(0, 5)}</span>
                  )}
                </div>
              </div>
            );

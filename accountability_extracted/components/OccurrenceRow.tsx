    ...
    <div className="rounded-xl bg-neutral-900 p-3 text-sm">
      <div className="mb-2 flex items-center justify-between">
        ...
        {occurrence.attendance_status && (
          <span className="rounded-full bg-neutral-800 px-2 py-0.5 text-sm capitalize text-neutral-300">
            {occurrence.attendance_status}
          </span>
        )}
      </div>
      ...
      <div className="mb-2.5 flex flex-wrap gap-2">
        {ATTENDANCE_OPTIONS.map((o) => (
          <button
            ...
            className={`min-h-[44px] flex-1 inline-flex items-center justify-center rounded-xl px-3 py-2 text-sm font-medium transition-colors disabled:opacity-50 ${
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 border-t border-neutral-800/60 pt-2">
        <div className="flex gap-2">
          <button
            ...
            className={`min-h-[44px] rounded-xl px-3.5 py-2 text-sm font-medium transition-colors disabled:opacity-50 ${
        ...
        <div className="flex items-center gap-1.5">
          <span className="text-xs text-neutral-500 mr-0.5">Focus:</span>
          {[1, 2, 3, 4, 5].map((n) => (
            <button
              key={n}
              type="button"
              disabled={pending}
              onClick={() => rate(n)}
              className={`h-9 w-9 rounded-xl text-sm font-medium flex items-center justify-center transition-colors disabled:opacity-50 ${

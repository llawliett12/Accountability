...
export default function TaskRow({
  ...
  return (
    ...
    >
        ...
        <div className="min-w-0 flex-1 pt-0.5">
          <div className="flex items-start justify-between gap-2">
            ...
            <div className="flex items-center gap-2.5 shrink-0">
              {isOptimistic ? (
                <span className="rounded-md bg-amber-950/60 border border-amber-800/40 px-2 py-0.5 text-xs font-mono text-amber-300">
                  saving...
                </span>
              ) : (
                <select
                  ...
                  className={`rounded-md bg-neutral-800/80 px-2 py-1 text-xs font-medium transition-colors outline-none cursor-pointer ${
                    isCompleted
          ...
          <div className="mt-1.5 flex flex-wrap items-center gap-1.5 text-sm text-neutral-400">
            {showPriorityBadge && task.is_top3 && (
              <span className="rounded bg-amber-950/60 border border-amber-800/50 px-1.5 py-0.5 text-xs font-semibold text-amber-300">
                ★ TOP 3
              </span>
            )}
            ...
            {goalTitle && (
              <span className="rounded bg-neutral-800/80 px-1.5 py-0.5 text-xs text-neutral-300 truncate max-w-[160px]">
                🎯 {goalTitle}
              </span>
            )}
          </div>
      ...
      {queuedLabel && <p className="mt-1.5 text-sm text-amber-400 pl-8">{queuedLabel}</p>}
      {error && <p className="mt-1.5 text-sm text-red-400 pl-8">{error}</p>}
    </li>
  );
}

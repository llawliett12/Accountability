...
export default function ScreenTimeCapture({ initialRecord = null }: { initialRecord?: { date: string; totalMinutes: number; source: "gemini" | "manual"; apps: AppUsage[] } | null }) {
    ...
    return (
      <div className="space-y-2 rounded-2xl bg-neutral-900 p-4">
        ...
        <button
          ...
          className="w-full rounded-lg bg-neutral-800 py-2 text-sm text-neutral-400"
        >
  ...
  return (
    <div className="space-y-3 rounded-2xl bg-neutral-900 p-4">
      ...
      {banner && <p className="rounded-lg bg-amber-950 p-2 text-sm text-amber-300">{banner}</p>}
      {saveError && (
      ...
      {warnings.map((w, i) => (
        <p key={i} className="rounded-lg bg-amber-950 p-2 text-sm text-amber-300">
          {w}
        </p>
      ))}

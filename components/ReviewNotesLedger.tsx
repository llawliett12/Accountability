          ...
          <tr className="align-top hover:bg-neutral-900/50 transition-colors">
            ...
            <td className="py-2 px-3 text-right whitespace-nowrap">
              <button
                ...
                className="min-h-9 rounded border border-amber-700/70 bg-amber-500/10 px-3 text-sm font-medium text-amber-300 transition-colors hover:bg-amber-500/20 disabled:cursor-not-allowed disabled:border-neutral-800 disabled:bg-transparent disabled:text-neutral-600"
              >
                {pending ? "Saving…" : isChanged ? "Save" : savedContent ? "Saved" : "Save"}
              </button>
              {savedContent && <button type="button" onClick={remove} disabled={pending} className="ml-2 min-h-9 text-sm text-red-400 disabled:opacity-50">Delete</button>}
            </td>
          </tr>
        </tbody>
      </table>

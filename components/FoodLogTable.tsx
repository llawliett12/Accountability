...
export default function FoodLogTable({
  ...
  return (
    <div className="space-y-4">
      ...
      >
        <div className="text-sm font-mono font-medium text-neutral-400 uppercase tracking-wider">
          Quick Food Entry
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-12 gap-2.5">
          <div className="sm:col-span-3">
            <label className="block text-xs font-mono text-neutral-500 mb-1">Time</label>
            <input
              type="time"
              value={time}
              onChange={(e) => setTime(e.target.value)}
              required
              className="w-full rounded-lg bg-neutral-950 border border-neutral-800 px-2.5 py-1.5 text-sm text-white font-mono focus:border-amber-400 focus:outline-none"
            />
          </div>
          <div className="sm:col-span-5">
            <label className="block text-xs font-mono text-neutral-500 mb-1">What I ate</label>
            <input
              type="text"
              value={food}
              onChange={(e) => setFood(e.target.value)}
              required
              className="w-full rounded-lg bg-neutral-950 border border-neutral-800 px-2.5 py-1.5 text-sm text-white placeholder-neutral-600 focus:border-amber-400 focus:outline-none"
            />
          </div>
          <div className="sm:col-span-3">
            <label className="block text-xs font-mono text-neutral-500 mb-1">Notes (optional)</label>
            <input
              type="text"
              value={notes}
          ...
          <div className="sm:col-span-1 flex items-end">
            <button
              ...
              className="w-full h-8 rounded-lg bg-amber-400 hover:bg-amber-300 text-neutral-950 text-sm font-semibold transition-colors disabled:opacity-50 flex items-center justify-center"
            >
              Add
            </button>
          </div>
      ...
      <div className="space-y-1.5">
        <div className="flex items-center justify-between border-b border-neutral-800/80 pb-1.5">
          ...
          <span className="font-mono text-xs text-neutral-500">[{entries.length}]</span>
        </div>
        ...
        <div className="overflow-x-auto border-y sm:border border-neutral-800/80 sm:rounded-xl bg-neutral-950/50">
          <table className="w-full text-left text-sm border-collapse min-w-[360px]">
            ...
            <tbody className="divide-y divide-neutral-800/60 font-mono text-sm">
              {entries.length === 0 ? (
                <tr>
                  <td colSpan={4} className="py-6 text-center text-sm text-neutral-500 font-sans">
                ...
                entries.map((item) => (
                  <tr key={item.id} className="hover:bg-neutral-900/40 transition-colors">
                    ...
                    <td className="py-2.5 px-3 text-neutral-400 font-sans text-xs">

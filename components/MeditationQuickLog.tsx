  ...
  return (
    <div className="space-y-4 rounded-xl border border-neutral-800 bg-neutral-900/60 p-4">
      <div className="flex items-center justify-between">
        ...
        {duration || happened ? (
          <span className="rounded-full bg-emerald-950/80 border border-emerald-800/60 px-3 py-1 font-mono text-xs font-semibold text-emerald-300">
            {duration ? `${duration} min logged` : "Logged ✓"}
          </span>
        ) : (
          <span className="rounded-full bg-neutral-900 border border-neutral-800 px-3 py-1 font-mono text-xs text-neutral-500">
            Not logged yet
          </span>
        )}
      ...
      <div className="space-y-2">
        <label className="text-xs font-mono text-neutral-400 block">Quick Log:</label>
        ...
        {showCustom && (
          <form onSubmit={handleCustomSubmit} className="flex gap-2 pt-2">
            ...
            <button
              ...
              className="rounded-lg bg-amber-400 px-4 py-1.5 text-xs font-semibold text-neutral-950 hover:bg-amber-300 transition-colors disabled:opacity-50"
            >
              Log
            </button>
          </form>
      ...
      {saved && <p className="text-xs text-emerald-400 font-mono">Meditation saved successfully ✓</p>}
